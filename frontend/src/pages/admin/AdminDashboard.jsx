...
const AdminDashboard = () => {
  ...
  const files = useMemo(() => {
    return (projects || []).flatMap(p => (p.files || []).map(f => ({ projectId: p._id, fileId: f._id, originalName: f.originalName, uploadedAt: f.uploadedAt, projectTitle: p.title, studentName: p.studentName })));
  }, [projects]);

  const filterdedFiles = files.filter(f => (f.originalName || "").toLowerCase().includes(reportSearch.toLowerCase()) || (f.projectTitle || "").toLowerCase().includes(reportSearch.toLowerCase()) || (f.studentName || "").toLowerCase().includes(reportSearch.toLowerCase()));
    ...
    <>
      <div className="space-y-6">
        ...
        <div className="bg-gradient-to-r from-blue-600 to-purple-700 rounded-lg p-6 text-white">
          ...
          <p className="text-blue-300">Manage the entire Project Management System and oversee all</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2  lg:grid-cols-5 gap-6">
          {
            dashboardStats.map((item, i) => {
              return (
                <div key={i} className={`p-2 ${item.bg} rounded-lg p-4`}>
                ...
                supervisorBucket.length === 0 ? (
                  ...
                  <div className="h-72 ">
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={supervisorBucket} margin={{ top: 8, right: 8, bottom: 8, left: 8 }} barCategoryGap={"20%"}>
                        <CartesianGrid strokeDasharray="3 3" stroke="#E2E8F0" vertical={false} />
                        <XAxis dataKey="name" tick={{ fontSize: 12, fill: "#334155" }} axisLine={{ stroke: "#CBD5E1" }} tickLine={{ stroke: "#CBD5E1" }} interval={0} height={50} dy={10} />
                        <YAxis allowDecimals={false} tick={{ fontSize: 12, fill: "#334155" }} axisLine={{ stroke: "#CBD5E1" }} tickLine={{ stroke: "#CBD5E1" }} />
                        <Tooltip cursor={{ fill: "rgba(99,102,241,0.05)" }} contentStyle={{ borderRadius: 8, borderColor: "#E2E8F0" }} formatter={(value, name) => [value, name === "count" ? "Projects Assigned" : name]}
                ...
                latestNotifications.map((n) => {
                  return (
                    <div key={n._id} className="flex items-center text-sm">
                      <div className={`mt-1 w-2 h-2 ${getBulletColor(n.type, n.priority)} rounded-full mr-3`} />

                      <div className="flex-1">
                        <p className="font-medium text-slate-800">{n.message}</p>
                        <div className="mt-1 flex items-center gap-2">
                          <span className={`px-2 py-0.5 rounded text-sm ront-medium ${getBadgeClasses("type"), String(n.type)}`}>
                            Type : {n.type}
                          </span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {
              actionButtons.map((btn, index) => {
                return (
                  <button key={index} className={`${btn.btnClass} flex items-center justify-center space-x-2`} onClick={btn.onClick}>
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 p-4">
              <div className="bg-white rounded-lg p-6 w-full max-w-3xl mx-4 max-h-screen overflow-y-auto">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-lg font-medium text-slate-500 ">All Files</h3>
                  <button onClick={() => setIsReportModelOpen(false)} className="text-slate-400 hover:text-slate-600" ><X className="w-5 h-5" /></button>
                </div>


                <div className="mb-4">
                  <input type="text" className="input w-full" placeholder="search by file name, project title, Student name" value={reportSearch} onChange={(e) => {
                    setReportSearch(e.target.value)
                  }} />
                </div>

                {
                  filterdedFiles.length === 0 ? (
                    <div className="text-slate-500 ">No Files Found</div>
                  ) : (
                    <div className="space-y-2">
                      {filterdedFiles.map((f, i) => {
                        return (
                          <div key={i} className="flex items-center  justify-between p-3 bg-slate-50 rounded">
                            <div>
                              <div className="font-medium text-slate-800 ">{f.originalName}
                              </div>
