...
const AssignSupervisor = () => {
  ...
  const teachers = useMemo(() => {
    ...
    return teacherUsers.map(t => ({
      ...t, assignedCount: Array.isArray(t.assignedStudents) ? t.assignedStudents.length : 0,
      capacityLeft:
  ...
  const Badge = ({ color, children }) => {
    return <span className={`inline-flex item-center px-2.5 py-0.5 rounded-full text-xs font-medium ${color}`}>{children}</span>
  }
    ...
    <div className="space-y-6">
      <div className="card">
        <div className="card-header">
          ...
          <p className="card-subtitle">Manage Supervisor assignments for students and projects </p>
        </div>
      </div>
                  ...
                  return (
                    <tr key={row.projectId} className="hover:bg-slate-50">
                        ...
                        <div>
                          {row.supervisor !== null ? (
                            ...
                            (<Badge
                              ...
                              children={row.status === "rejected" ? "Rejected " : " Not Assigned"} />)}
                        </div>
                      </td>
                      <td className="px-6 py-4">{row.deadline}</td>
                      <td className="px-6 py-4">{row.updatedAt}</td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <select className="input-field w-full" value={selectedSupervisor[row.projectId] || ""}
                          disabled={
                            !!row.supervisor ||
                            row.status === "rejected" ||
                            !row.isApproved} onChange={(e) => handleSupervisorSelect(
                          ...
                          {teachers
                            ...
                            .map((t) => (
                              <option value={t._id} key={t._id} >{t.name}
                                ({t.capacityLeft} slots left)</option>
                            ))
                          }
                        </select>
                      </td>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium ">
                        <button className="btn-primary text-sm w-[160px]" onClick={() => {
