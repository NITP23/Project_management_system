      ...
      <div className="card">
        <div className="card-header flex flex-col md:flex-row justify-between items-start md:items-center">
          <div>
            ...
            <p className="card-subtitle"> Create and monitor project deadlines</p>
          </div>
          <button onClick={() => setShowModel(true)} className="btn-primary mt-4 md:mt-0">Create/Update</button>
        </div>
      </div>
      ...
      <div className="card">
        <div className="flex flex-col md:flex-row gap-4">
          <div className="flex-1">
            <label className="block text-sm font-medium text-slate-700 mb-2">Search Deadlines</label>
            <input type="text" placeholder="Search by project or StudentName..." className="input-field w-full" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
                ...
                filteredProjects.map((row) => {
                  return (
                    <tr key={row._id} className="hover:bg-slate-50">
                      <td className="px-6 py-4 whitespace-npwrap">
                        <div>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">{row.supervisor !== "-" ? (
                        <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800">{row.supervisor}</span>

                      ) : (
                        <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-800">Not Assigned</span>
                      )}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 p-4">
          <div className="bg-white rounded-lg p-6 w-full max-w-3xl mx-4 max-h-screen overflow-y-auto">
            ...
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-medium text-slate-500 ">Create/Update Deadlines</h3>
              <button onClick={() => setShowModel(false)} className="text-slate-400 hover:text-slate-600" ><X className="w-5 h-5" /></button>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <input type="text" className="input" placeholder="Start typing to search projects...." value={query} onChange={(e) => { setQuery(e.target.value), setSelectedProject(null), setformData({ ...formData, projectTitle: e.target.value }) }} />
...

                {
                  query && !selectedProject && (
                    <div className="mt-2 border border-slate-200 rounded-md max-h-56 overflow-y-auto">
                      {
                        (projects || []).filter((p) => (p.title || "").toLowerCase().includes(query.toLowerCase()) || (p.student?.name || "").toLowerCase().includes(query.toLowerCase())).slice(0, 8).map((p) => (
                          <button type="button" key={p._id} className="w-full px-3 py-2 text-left hover:bg-slate-50" onClick={() => (setSelectedProject(p), setQuery(p.title), setformData({ ...formData, projectTitle: p.title, deadlineDate: p.deadline ? new Date(p.deadline).toString().slice(0, 10) : "" })
                          )}
                            title={p.title}>

                            <div className="text-sm font-medium text-slate-800 truncate">{p.title}</div>
                            <div className="text-xs text-slate-500 truncate">{p.student?.name || "-"}{"->"}{p.supervisor?.name || "-"}</div>
                          </button>
                        ))
                      }
                    </div>
                ...
                selectedProject && (
                  <div className="mt-4 border border-slate-200 rounded-lg p-4 bg-slate-50 ">
                    <div className="mb-2">
                      ...
                      <div className="text-sm truncate text-slate-700" title={selectedProject.description || ""}>{(selectedProject.description || '').length > 160 ? `${selectedProject.description.slice(0, 160)}...` : (selectedProject.description || "-")}</div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      ...
                      <div className="md:col-span-2">
                        ...
                        <div className="text-sm font-medium text-slate-800 ">{selectedProject.student?.name || "-"}-{selectedProject.student?.email || "-"}</div>
                      </div>
              ...
              <div className="flex justify-end space-x-3 pt-4" >
                <button type="button" onClick={() => setShowModel(false)} className="btn-secondary">Cancel</button>
