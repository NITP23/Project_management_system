...
const ProjectsPage = () => {
  ...
  const supervisors = useMemo(() => {
    const set = new Set(projects?.map(p => p?.supervisor?.name).filter(Boolean))
  ...
  const filteredProjects = projects?.filter(project => {
    ...
    const matchesSupervisor = filterSupervisor === "all" || project.supervisor?.name === filterSupervisor; 
  ...
  const files = useMemo(() => {
   ...
   const filteredFiles = files?.filter(
    (file) => 
    (file.originalName ||"")
    .toLowerCase().includes(reportSearch.toLowerCase()) ||
    (file.projectTitle || "")
    .toLowerCase().includes(reportSearch.toLowerCase())||
  ...
  const handleStatusChange = async(projectId, newStatus) => {
    if(newStatus === "approved"){
      ...
      // if(approvedProject.fulfilled.match(res)) 
      ...
      <div className="card">
          <div className="card-header flex flex-col md:flex-row justify-between items-start md:items-center">
            <div>
              ...
              <p className="card-subtitle">View and manage all the student's Projects across the platform</p>
            </div>
            <button onClick={() =>(setIsReportOpen(true))} className="btn-primary flex items-center space-x-2 mt-4 md:mt-0">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <div className="card">
          <div className="flex flex-col md:flex-row gap-4">
            ...
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-2">Filter Supervisors</label>
              <select className="input w-full" 
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-slate-50">
                <tr>
                  <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Project Details</th>
                  ...
                  <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Deadlines</th>
                    ...
                    return (
                      <tr key={project._id} className="hover:bg-slate-50 ">
                        ...
                        <td className="px-6 py-3 whitespace-nowrap">
                          <div className="text-sm font-medium text-slate-900">{project?.student?.name}</div>
                          <div className="text-xs text-slate-500">Last Updated: {project?.uploadedAt 
                        ...
                        <td className="px-6 py-3 whitespace-nowrap">
                         ...
                         <td className="px-6 py-3 whitespace-nowrap">
                          <span className={`inline-flex capitalize items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getStatusColor(project.status)}`}> {project.status}</span>
                        </td>
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 p-4">
              <div className="bg-white rounded-lg p-6 w-full max-w-2xl mx-4 max-h-screen overflow-y-auto">
                <div className="flex justify-between items-center mb-4">
                  ...
                  <button onClick={() => setShowViewModel(false)} className="text-slate-400 hover:text-slate-600">
                    <X className="w-6 h-6"/>
                  </button>
                </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grod-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <div className="input bg-slate-50">{currentProject?.student?.name || "-"}</div>
                    </div>
                    <div>
                      <label className="label">Supervisor</label>
                      <div className="input bg-slate-50">{currentProject?.supervisor?.name || "-"}</div>
                    </div>
                  </div>

                  <div className="grid grod-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <label className="label">Satus</label>
                      <div className="input bg-slate-50 capitalize">{currentProject?.status}</div>
                    </div>
                    <div>
                      <label className="label">Deadline</label>
                      <div className="input bg-slate-50">{currentProject?.deadline 
                      ? new Date(currentProject.deadline).toLocaleDateString() : "N/A"
                    }</div>
                    </div>
                  </div>
                          ...
                          <ul className="list-disc list-inside text-sm text-slate-700">
                            {
                              currentProject.files.map(file => (
                                <li key={file._id || file.fileUrl}> {file.originalName}
                                </li>
                              ))
                            }
                          </ul>
          ...
          isReportOpen && (
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 p-4">
              <div className="bg-white rounded-lg p-6 w-full max-w-2xl mx-4 max-h-screen overflow-y-auto">
                ...
                <div className="mb-4">
                  <input type="text" className="input w-full" placeholder="search by file name, project title or student name" value={reportSearch} onChange={(e) => setReportSearch(e.target.value)}/>
                </div>
                    ...
                    <div className="space-y-2">
                      {
                        filteredFiles.map(f => {
                          return (
                            <div key={`${f.projectId}-${f.fileId}`} className="flex items-center justify-between p-3 bg-slate-50 rounded">
                              <div >
                                <div className="font-medium text-slate-800">{f.originalName}</div>
                                <div className="font-sm text-slate-800">{f.projecTitle}-{f.studentName}</div>
                              </div>

                              <button 
                              onClick={() => handleDownloadFile(f) }
                              className="btn-outline btn-small" >Download</button>
                            </div>
                          )
                        })
                      }
