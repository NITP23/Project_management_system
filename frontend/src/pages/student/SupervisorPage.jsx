...
const SupervisorPage = () => {
  ...
  return <>
    <div className="space-y-6" >
              ...
              <div className="flex items-start space-x-6">
                ...
                <div className="flex-1 space-y-4">
                  <div>
                    ...
                    <p className="text-lg font-bold text-slate-600">{supervisor?.department || "-"}</p>
                  </div>

                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 ">
                    <div>
                      <label className="text-sm font-medium text-slate-500  upppercase tracking-wide"> Email</label>
                      <p className="text-slate-800 font-medium ">{supervisor?.email || "-"}</p>
                    </div>

                    <div>
                      <label className="text-sm font-medium text-slate-500  upppercase tracking-wide"> Expertise</label>
                      <p className="text-slate-800 font-medium ">{Array.isArray(supervisor?.experties) ? supervisor.experties.join(", ") : supervisor?.experties || "-"}</p>
                    </div>
      ...
      {/* project-details :  only show when it exists */}
            ...
            <div className="space-y-6 ">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-4">
                  ...
                  <div>
                    <label className="text-sm font-medium text-slate-500  uppercase tracking-wide">Project Status</label>
                    <div className="mt-1">
                      <span className={`inline-flex  items-center px-3 py-1 rounded-full font-medium capitalize text-sm ${project.status === "approved" ? "bg-green-100 text-green-800" : project.status === "pending" ? "bg-yellow-100 text-yellow-800" : project.status === "rejected" ? "bg-red-100 text-red-800" : "bg-gray-100 text-gray-800"} `}>
                ...
                <div className="space-y-4 ">
                  <div>
                    ...
                    <p className="text-lg font-semibold text-slate-800 mt-1">{project?.deadline ? formatDeadline(project.deadline) : "No deadline yet"}</p>
          ...
          <div className="card">
            ...
            <div className="p-6 text-center">
              <p className="text-slate-600 text-lg">
                You haven't submitted any project proposal yet. So you cannot request supervisor.
              </p>
            </div>
          </div>
        )
      ...
      {/* Available supervisor only when project exist and supervisor haven't assigned*/}
                ...
                supervisors && supervisors.map(sup => (
                  <div key={sup._id} className="border border-slate-200 rounded-lg p-4 hover:shadow-md transition-shadow ">
                    ...
                    <div className="space-y-2 mb-4">
                      <div>
                        {/* <label className="text-xs font-medium text-slate-500 "><Email></Email></label> */}
                      ...
                      <div>
                        ...
                        <p className="text-sm  text-slate-700 ">{Array.isArray(sup?.experties) ? sup.experties.join(", ") : sup?.experties || "-"}</p>
                      </div>
                    </div>
              ...
              <div className="bg-white rounded-lg p-6 w-full max-w-md shadow-lg">
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <textarea className="input min-h-[120px]" required value={requestMessage} onChange={(e) => setRequestMessage(e.target.value)} placeholder="Introduce yourself and why you would write  to superv ise your project..." />
                  </div>

                  <div className="flex justify-end space-x-3 pt-4 border-t border-slate-200">
                    <button onClick={() => {
                      setShowRequestModal(false); setSelectedSupervisor(null); setRequestMessage("");
